...
struct StudentOutDetailView: View {
    let student: Student
    ...
    var leavingOutcomes: [String] {
        return reasonStore.signOutReasons
    }
    
    var isLeaving: Bool {
        return selectedOutcome == "Other" || leavingOutcomes.contains(selectedOutcome)
    }
    ...
    func confirmSignOut() {
        ...
        let logEntry = "\(student.fullName),\(student.year),\(student.teacher),\(finalOutcome),\(roomType.rawValue),\(timestamp),SIGN_OUT\n"
